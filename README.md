...
A type with a single variant may have the `unboxed` qualifier, which means its fields will be inlined into arrays, datatypes, argument lists, and local variables. Note that when the type is used in a polymorphic context, inlining is not guaranteed.
